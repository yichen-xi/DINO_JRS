...
class NMI(torch.nn.Module):
    ...
    def nmi(self, y_true, y_pred):
        mi = self.mi(y_true, y_pred)  # MI
        # Compute marginal entropies
        pa = torch.mean(torch.exp(-self.preterm * torch.square(y_true - self.vol_bin_centers)), dim=1, keepdim=True)
        pb = torch.mean(torch.exp(-self.preterm * torch.square(y_pred - self.vol_bin_centers)), dim=1, keepdim=True)

        ha = -torch.sum(pa * torch.log(pa + 1e-6), dim=-1)
        hb = -torch.sum(pb * torch.log(pb + 1e-6), dim=-1)

        nmi = 2 * mi / (ha.mean() + hb.mean())
        return nmi
...
def recon_loss(y_pred,y_true,image_sigma=0.01):
    """ reconstruction loss """
    return 1. / (image_sigma ** 2) * torch.mean((y_true - y_pred)**2)  ##!!!!!!!!
