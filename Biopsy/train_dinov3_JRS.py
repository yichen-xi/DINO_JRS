...
from Biopsy.models.DINO_JRS import dinov3_JRS
import random
...
def main():
    batch_size = 1
    # train_dir = '/SSFS/shskw25_1/gyl/DataSets/Biopsy/train'
    # val_dir = '/SSFS/shskw25_1/gyl/DataSets/Biopsy/valid'

    train_dir = '/home/gyl/DataSets/Biopsy/train'
    val_dir = '/home/gyl/DataSets/Biopsy/valid'
    lr = 0.0001
    weights = [0.01, 0.01, 1, 1, 1, 1, 1]  # loss weights
    save_dir = 'dinov3_JRS_ncc_{}_weakly_{}_diffusion_{}_lr_{}/'.format(weights[0], weights[2],weights[6],lr)
    ...
    Initialize model
    '''
    model = dinov3_JRS(img_size,n_classes=1,target_size=(256,256))
    # model = dinov3_JRS(img_size,n_classes=1,target_size=(2048,2048))
    model.cuda()
    ...
    criterions = [losses.NCC_vxm(),losses.NCC_vxm(),losses.Grad3d(penalty='l2'),losses.Grad3d(penalty='l2')
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            y_seg = data[3]
            output = model(x,y)
            def_x_seg = reg_model_bilin([x_seg, output[2]])
            # flow_inv=output[4]
            # def_y_seg = reg_model_bilin([y_seg, output[4]])
            loss = 0
            loss_vals = []
            labels = [y,x,0,0,x_seg,y_seg]
            ...
            # loss+=seglossy
            aux_loss=segloss(def_x_seg.float(),y_seg.float())*weights[6]
            loss+=aux_loss
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                # segx= torch.sigmoid(output[4])
                # segy = torch.sigmoid(output[5])
